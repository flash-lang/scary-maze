...
(function( $ ){
  ...
  */
  $.fn.hitTestPoint = function(options) {
	...
	var imageData = ctx.getImageData(settings.x - objectRect.x, settings.y - objectRect.y, settings.radius, settings.radius);
	
	if (settings.radius > 1){
	
	var pixelFound = 0;
	for (i = 0; i < imageData.data.length; i += 4) {
		if (imageData.data[i+3] == 255) pixelFound = 1;
		if (imageData.data[i+1] == 255) pixelFound = 2;
	}
    return pixelFound; 
	}else
	{
